...
public enum ScoredItem {
    ...
    public int score(List<Integer> dice) {
        int score = switch (this) {
            case ACES, TWOS, THREES, FOURS, FIVES, SIXES -> frequency(dice) * faceValue;
            case THREE_OF_KIND -> getMax(dice) >= 3 ? dice.stream().mapToInt(i -> i).sum() : 0;
            case FOUR_OF_KIND -> getMax(dice) >= 4 ? dice.stream().mapToInt(i -> i).sum() : 0;
            case FIVE_OF_KIND -> getMax(dice) == 5 ? defaultScore : 0;
            case SMALL_STRAIGHT -> checkStraight(dice) >= 4 ? defaultScore : 0;
            case LARGE_STRAIGHT -> checkStraight(dice) == 5 ? defaultScore : 0;
            case FULL_HOUSE -> checkFullHouse(dice) ? defaultScore : 0;
